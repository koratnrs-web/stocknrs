...
export function AddBudgetRequestDialog({ onSuccess, editRequest }: AddBudgetRequestDialogProps) {
  ...
  const sendEmailJS = async (requestData: BudgetRequestForm, approverName: string, approverEmail: string, ccEmails: string[], requestId: string) => {
    ...
    try {
      ...
      if (error.status === 404) {
        ...
        throw new Error(`EmailJS 400 Error: ข้อมูลที่ส่งไม่ถูกต้อง - ${error.text || error.message}`);
      ...
      throw new Error(`EmailJS Error (${error.status || 'Unknown'}): ${error.text || error.message || 'Unknown error'}`);
    }
  };
